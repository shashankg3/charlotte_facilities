# =============================================================
# V5_hyperparam_search_fl.py  (Food Lion study)
#
# Grid-search over (radius_miles, beta, K, W1, W2, W3) to find
# the combination that best recovers actual Food Lion store
# locations — i.e. maximises the mean percentile rank of the
# 27 real FL stores among all 2,450 scored candidates.
#
# Two-stage search:
...
# USAGE:
#   cd <project_root>
#   C:\Python312\python.exe validations_foodlion/V5/V5_hyperparam_search_fl.py
#
# OUTPUT:
#   Results_final/FL_results/V5/hyperparam_results.json
#   Results_final/FL_results/V5/hyperparam_report.txt
# =============================================================
...
MAX_MATCH_MI = 0.5          # FL store matched if within 0.5 mi of a candidate
...
def _mean_pct_rank(state, radius, beta, K, W1, W2, W3, fl_xy, cand_tree):
    """
    Score all candidates, match FL stores, return mean percentile rank.
    Higher = FL stores are ranked better by the model.
    Returns NaN if fewer than 5 FL stores can be matched.
    """
    try:
    ...
    # Match FL stores → nearest candidate
    cands_xy  = np.c_[state["cands_m"].geometry.x.values,
...
def main():
    ...
    print("=" * 65)
    print("  V5: HYPERPARAMETER SEARCH — Food Lion")
    print("=" * 65)

    # ── Load model state once ────────────────────────────────
    print("\n[1/3] Loading Food Lion model state...")
    t0    = time.perf_counter()
    state = load_all()
    fl_m  = state["ht_m"]
    print(f"      Candidates: {len(state['cands_m'])}, "
          f"FL stores: {len(fl_m)}")
    ...
    for i, (r, b, k) in enumerate(combos_s1, 1):
        mpr = _mean_pct_rank(state, r, b, k, 0.4, 0.3, 0.3, fl_xy, cand_tree)
        s1_results.append({
            "radius": r, "beta": b, "K": k,
    ...
    print(f"      Mean FL percentile: {best_s1_score:.2f}")
    ...
    print(f"  Mean FL pctile   : {best_s2_score:.2f}th")
    ...
    json_path = os.path.join(RESULTS_DIR, "hyperparam_results.json")
    ...
    # Plain-text report
    report_lines = [
        "V5 HYPERPARAMETER SEARCH REPORT — Food Lion, Charlotte NC",
        "=" * 65,
        "",
        "BEST COMBINATION",
        ...
        f"  W2 (income)      : {best_w2}",
        f"  W3 (access)      : {best_w3}",
        f"  Mean FL pctile   : {best_s2_score:.2f}th (higher = better)",
        "",
        "INTERPRETATION",
        "  Mean percentile rank measures how high the model scores",
        "  actual Food Lion locations vs all 2,450 candidates.",
        "  A value of 75 means the average real FL store falls in",
    ...
    txt_path = os.path.join(RESULTS_DIR, "hyperparam_report.txt")
