# =============================================================
# V5_hyperparam_search_2_ht.py  (Harris Teeter study)
#
...
#   then found optimal weights separately.
...
#
# OUTPUT:
#   Results_final/V5_results/hyperparam_search_2_results.json
#   Results_final/V5_results/hyperparam_search_2_report.txt
#
# USAGE:
#   cd <project_root>
#   C:\Python312\python.exe validations/V5/V5_hyperparam_search_2_ht.py
# =============================================================
...
def mean_pct_rank(state, radius, beta, K, W1, W2, W3, ht_xy, cand_tree):
    """Return mean percentile rank of matched HT stores. Higher = better."""
...
def search_huff_params(state, W1, W2, W3, ht_xy, cand_tree, label=""):
    ...
    for i, (r, b, k) in enumerate(combos, 1):
        mpr = mean_pct_rank(state, r, b, k, W1, W2, W3, ht_xy, cand_tree)
...
def search_weights(state, radius, beta, K, ht_xy, cand_tree, label=""):
    ...
    for i, (w1, w2, w3) in enumerate(combos, 1):
        mpr = mean_pct_rank(state, radius, beta, K, w1, w2, w3, ht_xy, cand_tree)
...
def main():
    ...
    print("=" * 65)
    print("  V5 SEARCH 2: COORDINATE DESCENT — Harris Teeter")
    print("=" * 65)

    # Load model once
    print("\n[LOAD] Loading Harris Teeter model state...")
    t0    = time.perf_counter()
    state = load_all()
    ht_m  = state["ht_m"]
    cands = state["cands_m"]
    ...
    r1a_params, r1a_score, r1a_res = search_huff_params(
        state, 0.4, 0.3, 0.3, ht_xy, cand_tree, "R1-A  Huff params")
    all_results.extend(r1a_res)

    r1b_w, r1b_score, r1b_res = search_weights(
        state, *r1a_params, ht_xy, cand_tree, "R1-B  Weights")
    ...
    r2a_params, r2a_score, r2a_res = search_huff_params(
        state, *r1b_w, ht_xy, cand_tree, "R2-A  Huff params")
    all_results.extend(r2a_res)

    r2b_w, r2b_score, r2b_res = search_weights(
        state, *r2a_params, ht_xy, cand_tree, "R2-B  Weights")
    ...
    print(f"  Mean HT pctile   : {best['mean_pct_rank']}th")
    print(f"  Converged        : {'Yes' if converged else 'No — consider search_3'}")
    print(f"{'=' * 65}")

    # ── Round comparison ─────────────────────────────────────
    print(f"\n  Improvement over search_1:")
    print(f"    search_1 best : 60.63th pctile  (r=4.0 b=3.0 K=3 W1=0.6 W2=0.3 W3=0.1)")
    ...
    report = [
        "V5 HYPERPARAM SEARCH 2 — Coordinate Descent — Harris Teeter Charlotte",
        "=" * 65,
        "",
        ...
        f"  Mean HT percentile: {best['mean_pct_rank']}th",
        "",
        "TOP-10",
