# =============================================================
# V5_hyperparam_search_5_ht.py  (Harris Teeter study)
#
# SEARCH 5 — Extended grid below radius=2.5 and above K=7
#
# WHY SEARCH 5:
#   Search 4 winner hit TWO grid edges:
#     radius = 2.5  (bottom of 2.5–4.5 grid)
#     K      = 7    (top of 5–7 grid)
#   Search 5 extends both and re-runs LOOCV.
#
# NEW GRIDS:
#   radius : 1.0, 1.5, 2.0, 2.5, 3.0  (extends below 2.5)
#   beta   : 3.5, 4.0, 4.5             (centred on search_4 winner 4.0)
#   K      : 7, 8, 9, 10               (extends above 7)
#   W1     : 0.65, 0.70, 0.75, 0.80, 0.85  (search_4 winner 0.75 was interior)
#   W2     : 0.05, 0.10, 0.15, 0.20
#
# PIPELINE: identical to search_3 / search_4
...
#
# OUTPUT:
#   Results_final/V5_results/hyperparam_search_5_results.json
#   Results_final/V5_results/hyperparam_search_5_report.txt
#   Results_final/V5_results/optimal_params.json  (updated if improved)
#
# USAGE:
#   cd <project_root>
#   C:\Python312\python.exe validations/V5/V5_hyperparam_search_5_ht.py
# =============================================================
...
# ── Extended grids ────────────────────────────────────────────
RADIUS_GRID = [1.0, 1.5, 2.0, 2.5, 3.0]   # extends below 2.5
BETA_GRID   = [3.5, 4.0, 4.5]              # centred on search_4 winner 4.0
K_GRID      = [7, 8, 9, 10]               # extends above 7

W1_GRID = [0.65, 0.70, 0.75, 0.80, 0.85]  # search_4 winner 0.75 was interior
W2_GRID = [0.05, 0.10, 0.15, 0.20]

# Search_4 reference for comparison
SEARCH4_LOOCV  = 65.363
SEARCH4_PARAMS = "r=2.5 b=4.0 K=7 W1=0.75 W2=0.10 W3=0.15"


# =============================================================
# HELPERS  (identical to search_3/4)
# =============================================================
...
def main():
    ...
    print("=" * 65)
    print("  V5 SEARCH 5: Extended Grid + LOOCV — Harris Teeter")
    print("=" * 65)
    print(f"  Search 4 reference: LOOCV={SEARCH4_LOOCV}th  ({SEARCH4_PARAMS})")
    print(f"  Search 5 extends: radius down to 1.0mi, K up to 10")
    ...
    W1_FIXED, W2_FIXED, W3_FIXED = 0.75, 0.10, 0.15   # search_4 winner
    ...
    # Convergence check on radius and K
    ...
    if at_k_edge:
        ...
        print(f"  WARNING: K={best_k} is at grid edge ({edge_dir})")
    else:
        print(f"  K={best_k} is interior to grid -- good")
    ...
    converged  = (not at_radius_edge) and (not at_k_edge) and (not at_w1_edge)
    ...
    improved = best_final_score > SEARCH4_LOOCV
    ...
    print(f"    search_4 (LOOCV ext.) : {SEARCH4_LOOCV}th pctile")
    print(f"    search_5 (LOOCV ext.) : {best['loocv_pct_rank']}th pctile  "
    ...
    if converged:
        ...
        if at_k_edge:
            ...
            print(f"  Consider extending K grid further {edge_dir}.")
        if at_w1_edge:
    ...
    output = {
        "brand":        "Harris Teeter",
        "city":         "Charlotte, NC",
        "test":         "hyperparam_search_5_loocv_extended",
        "best_params":  best,
        "converged":    converged,
        "improved_over_search4": improved,
    ...
    json_path = os.path.join(RESULTS_DIR, "hyperparam_search_5_results.json")
    with open(json_path, "w") as f:
        json.dump(output, f, indent=2)

    report = [
        "V5 HYPERPARAM SEARCH 5 -- Extended Grid + LOOCV -- Harris Teeter",
        "=" * 65,
        "",
        "WHY SEARCH 5:",
        "  Search 4 winner hit grid edges: radius=2.5 (bottom) and K=7 (top)",
        "  Search 5 extends radius down to 1.0mi and K up to 10",
        "",
        ...
        "  search_2 : 60.958th",
        "  search_3 : 64.706th",
        f"  search_4 : {SEARCH4_LOOCV}th",
        f"  search_5 : {best['loocv_pct_rank']}th  ({'improved' if improved else 'no improvement'})",
        "",
        "TOP-10 WEIGHT COMBOS (LOOCV-ranked)",
    ...
    txt_path = os.path.join(RESULTS_DIR, "hyperparam_search_5_report.txt")
    ...
    if improved:
        optimal = {
            "source":         "V5_hyperparam_search_5_loocv_extended",
            "radius_miles":   best_r,
            "beta":           best_b,
            "K":              best_k,
            "W1":             best_w1,
        ...
        print(f"\n  Updated: {opt_path}  (improved over search_4)")
    else:
        print(f"\n  optimal_params.json NOT updated (search_4 still better)")
