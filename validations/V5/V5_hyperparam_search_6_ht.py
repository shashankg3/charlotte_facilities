# =============================================================
# V5_hyperparam_search_6_ht.py  (Harris Teeter study)
#
# SEARCH 6 — Extended grid above radius=3.0 and below K=7
#
# WHY SEARCH 6:
#   Search 5 winner hit TWO grid edges:
#     radius = 3.0  (top of 1.0–3.0 grid)
#     K      = 7    (bottom of 7–10 grid)
#   Search 6 extends both and re-runs LOOCV.
#
# NEW GRIDS:
#   radius : 2.5, 3.0, 3.5, 4.0  (extends above 3.0)
#   beta   : 4.0, 4.5, 5.0       (extends above search_5 winner 4.5)
#   K      : 5, 6, 7, 8          (extends below 7)
#   W1     : 0.70, 0.75, 0.80, 0.85  (search_5 winner 0.80 was interior)
#   W2     : 0.05, 0.10, 0.15, 0.20
#
# PIPELINE: identical to search_3/4/5
...
#
# OUTPUT:
#   Results_final/V5_results/hyperparam_search_6_results.json
#   Results_final/V5_results/hyperparam_search_6_report.txt
#   Results_final/V5_results/optimal_params.json  (updated if improved)
#
# USAGE:
#   cd <project_root>
#   C:\Python312\python.exe validations/V5/V5_hyperparam_search_6_ht.py
# =============================================================
...
# ── Extended grids ────────────────────────────────────────────
RADIUS_GRID = [2.5, 3.0, 3.5, 4.0]        # extends above 3.0
BETA_GRID   = [4.0, 4.5, 5.0]             # extends above search_5 winner 4.5
K_GRID      = [5, 6, 7, 8]               # extends below 7

W1_GRID = [0.70, 0.75, 0.80, 0.85]        # search_5 winner 0.80 was interior
W2_GRID = [0.05, 0.10, 0.15, 0.20]

# Search_5 reference for comparison
SEARCH5_LOOCV  = 66.072
SEARCH5_PARAMS = "r=3.0 b=4.5 K=7 W1=0.80 W2=0.05 W3=0.15"


# =============================================================
# HELPERS  (identical to search_3/4/5)
# =============================================================
...
def main():
    ...
    print("=" * 65)
    print("  V5 SEARCH 6: Extended Grid + LOOCV — Harris Teeter")
    print("=" * 65)
    print(f"  Search 5 reference: LOOCV={SEARCH5_LOOCV}th  ({SEARCH5_PARAMS})")
    print(f"  Search 6 extends: radius above 3.0mi, K below 7, beta above 4.5")
    ...
    W1_FIXED, W2_FIXED, W3_FIXED = 0.80, 0.05, 0.15   # search_5 winner
    ...
    at_k_edge      = (best_k == min(K_GRID)      or best_k == max(K_GRID))
    at_beta_edge   = (best_b == min(BETA_GRID)   or best_b == max(BETA_GRID))
    for name, flag, val, grid in [
        ("radius", at_radius_edge, best_r, RADIUS_GRID),
        ("K",      at_k_edge,      best_k, K_GRID),
        ("beta",   at_beta_edge,   best_b, BETA_GRID),
    ]:
        if flag:
            edge_dir = "below" if val == min(grid) else "above"
            print(f"  WARNING: {name}={val} is at grid edge ({edge_dir})")
        else:
            print(f"  {name}={val} is interior to grid -- good")
    ...
    converged  = (not at_radius_edge) and (not at_k_edge) and \
                 (not at_beta_edge)   and (not at_w1_edge)
    ...
    improved = best_final_score > SEARCH5_LOOCV
    ...
    print(f"    search_3 (LOOCV)      : 64.706th pctile")
    print(f"    search_4 (LOOCV ext.) : 65.363th pctile")
    print(f"    search_5 (LOOCV ext.) : {SEARCH5_LOOCV}th pctile")
    print(f"    search_6 (LOOCV ext.) : {best['loocv_pct_rank']}th pctile  "
    ...
    if converged:
        ...
        if at_k_edge:
            edge_dir = "above" if best_k == max(K_GRID) else "below"
            print(f"  NOT CONVERGED: K still at grid edge ({edge_dir}).")
        if at_beta_edge:
            edge_dir = "above" if best_b == max(BETA_GRID) else "below"
            print(f"  NOT CONVERGED: beta still at grid edge ({edge_dir}).")
        if at_w1_edge:
    ...
    output = {
        "brand":        "Harris Teeter",
        "city":         "Charlotte, NC",
        "test":         "hyperparam_search_6_loocv_extended",
        "best_params":  best,
        "converged":    converged,
        "improved_over_search5": improved,
    ...
    json_path = os.path.join(RESULTS_DIR, "hyperparam_search_6_results.json")
    with open(json_path, "w") as f:
        json.dump(output, f, indent=2)

    report = [
        "V5 HYPERPARAM SEARCH 6 -- Extended Grid + LOOCV -- Harris Teeter",
        "=" * 65,
        "",
        "WHY SEARCH 6:",
        "  Search 5 winner hit grid edges: radius=3.0 (top) and K=7 (bottom)",
        "  Search 6 extends radius above 3.0mi and K below 7",
        "",
        ...
        "  search_3 : 64.706th",
        "  search_4 : 65.363th",
        f"  search_5 : {SEARCH5_LOOCV}th",
        f"  search_6 : {best['loocv_pct_rank']}th  ({'improved' if improved else 'no improvement'})",
        "",
        "TOP-10 WEIGHT COMBOS (LOOCV-ranked)",
    ...
    txt_path = os.path.join(RESULTS_DIR, "hyperparam_search_6_report.txt")
    ...
    if improved:
        optimal = {
            "source":         "V5_hyperparam_search_6_loocv_extended",
            "radius_miles":   best_r,
            "beta":           best_b,
            "K":              best_k,
            "W1":             best_w1,
        ...
        print(f"\n  Updated: {opt_path}  (improved over search_5)")
    else:
        print(f"\n  optimal_params.json NOT updated (search_5 still better)")
